...
const ACTIVE_HINT_ELEMENT_CLASS = 'CodeMirror-hint-active';
 ...
 declare module 'codemirror' {
    let commands: any;
    let hint: any;
    function showHint(cm: CodeMirror.Editor, hinter?: HintFunction, options?: ShowHintOptions);
    ...
    type HintFunction = (cm: CodeMirror.Editor) => Hints;
...
const requestAnimationFrame: any = window.requestAnimationFrame || function (fn) {
...
(CodeMirror as any).showHint = function (cm, getHints, options) {
...
class Completion {
    ...
    pick(data, i: number): void {
        const completion = data.list[i];
        if (completion.hint) completion.hint(this.cm, data, completion);
        else {
            if (completion.pick) {
                completion.pick(this.cm, completion, data);
            } else {
...
class Widget {
    ...
    constructor(completion, data) {
        ...
        if (scrolls) {
            for (let node: any = hints.firstChild; node; node = node.nextSibling) {
                node.style.paddingRight = cm.display.nativeBarWidth + 'px';
            }
        }

        cm.addKeyMap(this.keyMap = this.buildKeyMap(completion, {
        ...
        if (completion.options.closeOnUnfocus) {
            let closingOnBlur;
            cm.on('blur', this.onBlur =  () => { closingOnBlur = setTimeout(function () { completion.close(); }, 100); });
        ...
        CodeMirror.on(hints, 'dblclick',  (e) => {
            const t = this.getHintElement(hints, e.target || e.srcElement);
    ...
    buildKeyMap(completion: Completion, handle: CodeMirror.Handle): any {
        const baseMap = {
    ...
    getHintElement(hintsElement, el: HTMLLIElement | Node): Node {
...
function parseOptions(cm: CodeMirror.Editor, pos, options) {
    const editor = cm.options.hintOptions;
    const out: { hint?} = {};
    for (const prop in defaultOptions) out[prop] = defaultOptions[prop];
    if (editor) {
...
function resolveAutoHints(cm, pos) {
    ...
    let words;
    if (helpers.length) {
        const resolved: any =  (cm1, callback, options) => {
            const app = applicableHelpers(cm1, helpers);
            function run(i) {
                if (i === app.length) return callback(null);
                fetchHints(app[i], cm1, options, function (result) {
        ...
        return (cm1) => CodeMirror.hint.fromList(cm1, { words: words });
    } else if (CodeMirror.hint.anyword) {
        return (cm1, options) => CodeMirror.hint.anyword(cm1, options);
    } else {
        return () => ({});
    }
}
...
CodeMirror.defineOption('hintOptions', null, null);
